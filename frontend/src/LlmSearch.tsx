...
//   if (typeof results === 'string') {
    // Display a read-only text area
    return (
      <Typography variant="body1" component="pre">{results}</Typography>
    );
//   }
    // return <>
    //     {results?.map((chunk) => <RecordContextProvider key={chunk.chunk_id} value={chunk}>
...
export const LlmSearch: React.FC<LlmSearchComponentProps> = () => {
    ...
    const handleLlmSearch = async () => {
        ...
        // })
        // setResults(json);
        setResults(chatCompletion?.data?.choices?.map((record) => { return record?.message?.content ? record?.message?.content : "" }));
    };
